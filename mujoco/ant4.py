...
tau = 0.9
device = "cuda:0"
weight_file = os.path.dirname(__file__) + "/weights/" + os.path.basename(__file__).split(".")[0] + "_4.pt"
...
add_arg("--trajectories-limit", type=int, default=20, help="")
...
class Policy(nn.Module):
    def __init__(self, obs_count, action_count, usebn=False):
        ...
        self.logstd = nn.Parameter(torch.zeros((1, action_count)), requires_grad=True)  # trainable parameter
...
def train(
    ...
    ret2 = 0
    if True: #train_policy and train.steps % 3 == 0:
        mu, std = actor(s)
        ...
        critic1_target.requires_grad_(False)
        advantage = v1_target - critic1_target(s)
...
def show():
    ...
    pi.load_state_dict(torch.load('weights/ant3_1.pt', map_location=torch.device(device)))
    pi.to(device)
    for runs in range(10):
        ...
        action = 0
        while True:
            action, log_prob = select_action(pi, s, None, None)
            ...
            if done:
                break
            total_steps += r
        print(f"#{runs} done, steps:{steps}, total_steps:{total_steps}", info)
    env.close()
...
def main():
    ...
    env = gym.make(ENV_NAME, healthy_z_range=(0.3, 1))
    ...
    torch.nn.init.constant_(pi.logstd, -0.5)
    ...
    for n_epi in range(1, args.max_episodes):
        ...
        for i in range(play_cnt):
            ...
            while not (done or truncated):
                ...
                # if need_render:
                #     env.render()
                r = _r + info["reward_forward"] * 10 # - 0.1
        ...
        update_target_net(vnet_target, vnet, 0.0)
        ...
        if n_epi % print_interval == 0:
            ...
            print(
                ...
                f"avg x_pos : {np.array(x_pos).mean():05.2f}, "
