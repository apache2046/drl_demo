...
def main():
    ...
    pi = Policy(action_count-1, usebn=True)
    #pi.load_state_dict(torch.load(weight_file))
    pi_target = Policy(action_count-1, usebn=True)
    pi_target.load_state_dict(pi.state_dict())

    Q1 = ActionValue(action_count-1, usebn=True)
    Q1_target = ActionValue(action_count-1, usebn=True)
    Q1_target.load_state_dict(Q1.state_dict())

    Q2 = ActionValue(action_count-1, usebn=True)
    Q2_target = ActionValue(action_count-1, usebn=True)
    ...
    for n_epi in range(1, args.max_episodes):
        ...
        for i in range(play_cnt):
            ...
            while not done:
                ...
                if n_epi < 30:
                    action = np.random.normal(0, 0.2, size=2).clip(-0.9, 0.9)
                elif steps == 0:
                    ...
                    shift_action = np.random.normal(0, 0.1, size=2) * noise_ratio
                    action = (action + shift_action).clip(-0.9, 0.9)
                ...
                for _ in range(1):  # range(min(1, 10 - n_epi // 100)):
                    ...
                    if acc > 0:
                        ...
                        brake = -acc
                    obs_new, _r, done, truncated, info = env.step([steer, throttle, brake])
