...
add_arg("--resume", type=str, default="", help="")
...
def get_random_traj(msg_q):
    ...
    while True:
        ...
        while True:
            ...
            traj.append((diff_obs, action, reward - action * 0.2, diff_obs_new, done_mask))
...
q = QNet()
if args.resume != "":
    print('resuming with', args.resume)
    q.load_state_dict(torch.load(args.resume, map_location=torch.device(device)))
    for param in q.cnn.parameters():
        param.requires_grad = False
q_target = QNet()
